...
#### Adding a whole room at once

The device list in the automatic setup choice takes more than one device, and an area can be chosen alongside it. Every device selected is added in one go under the name it has in the app and filed in that area, so a room's worth of bulbs takes one pass instead of one pass each.

Selecting several devices always adds them without further questions, because there is no way to ask about each one in turn. Any device that would have needed an answer — one whose key is not known, one that does not answer, or one whose configuration is not obvious — is left out and named in the summary at the end, so you can add it on its own.

The area is only a suggestion, which Home Assistant takes when the device first registers itself. Moving a device to another area afterwards is not undone by it.
